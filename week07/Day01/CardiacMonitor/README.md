...
- Transactional medication refill ordering with stock checks and calculated totals
...
- A patient can have multiple medication orders, each containing one or more medication items.
...
| Method | Endpoint | Access | Description |
| --- | --- | --- | --- |
| `POST` | `/api/patients/{patientId}/medication-orders` | Admin, Doctor | Create a refill order, calculate totals, and decrement stock atomically |

Medication order creation rejects missing, inactive, or insufficiently stocked medications. It stores the order and its items and decrements all requested medication stock inside one serializable EF Core transaction.
...
Run the complete unit and integration test suite from the `CardiacMonitor` directory:

```powershell
dotnet test .\CardiacMonitor.slnx --configuration Release
```

The suite uses xUnit and Moq for unit tests and `WebApplicationFactory<Program>` with an isolated EF Core InMemory database for integration tests. It covers authentication and refresh-token replay prevention, patient ownership authorization, clinical vital-sign validation, authenticated patient endpoint happy and error paths, and safe global exception handling.

See [WEEK6_SPRINT1_SUMMARY.md](WEEK6_SPRINT1_SUMMARY.md) for what shipped in Sprint 1, the Postman demo, the ERD, migration history, what carries into Sprint 2, and the retrospective.

Daily implementation notes:

- [Day 4 - Write Operations, Business Logic, and Code Review](docs/day04/README.md)
- [Day 5 - Sprint Review, Postman Demo, and Retrospective](docs/day05/README.md)
